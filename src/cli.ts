...
async function handleCliOnly(command: string, args: string[]) {
  ...
  if (command === 'claim' || command === 'claims') {
  ...
  if (command === 'actions') {
    const { runActionsCommand } = await import('./commands/actions.ts');
    await runActionsCommand(null, args);
    return;
  }
  if (command === 'scout') {
  ...
  if (command === 'world') {
    const { runWorldCommand } = await import('./commands/world.ts');
    await runWorldCommand(null, args);
    return;
  }
  if (command === 'context') {
    const { runContextCommand } = await import('./commands/context.ts');
    await runContextCommand(null, args);
    return;
  }
  if (command === 'personal') {
    const { runMemory } = await import('./commands/memory.ts');
